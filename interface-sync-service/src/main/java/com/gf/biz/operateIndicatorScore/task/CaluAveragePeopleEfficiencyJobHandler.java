...
public class CaluAveragePeopleEfficiencyJobHandler extends IJobHandler {
    private static final Logger logger = LoggerFactory.getLogger(SyncElemeShopRatingInfoDtlJobHandler.class);
    private static final String PI_CODE = "PI0017";
    private static final String PI_NAME = "平均人效";
    private static final BigDecimal dimensionWeight = new BigDecimal("0.29");
    private static final BigDecimal firstLevelIndicatorWeight = new BigDecimal("0.45");
    private static final String status = "1";//代表生效
    private static final String bdItemCode = "J30004";
    ...
    public void execute() throws Exception{
        ...
        if (jobDeptCode != null) {
            ...
            if (deptList != null && deptList.size() > 0) {
                for (LcapDepartment4a79f3 dept : deptList) {
                    this.getAveragePeopleEfficiencyScore(dept, currentYear, currentMonth);
                }
            }
        ...
        if (optDeptList != null && optDeptList.size() > 0) {
            for (LcapDepartment4a79f3 dbDept : optDeptList) {
                ...
                this.getAveragePeopleEfficiencyScore(dbDept, currentYear, currentMonth);
            }
        }

    }
    public void getAveragePeopleEfficiencyScore(LcapDepartment4a79f3 currentDept, Integer year, Integer month) throws Exception{
        ...
        if (bfBusinessAllProceedsList != null && bfBusinessAllProceedsList.size() > 0) {
            ...
            if(planValue.compareTo(BigDecimal.ZERO)==0){
                ...
                if(achieveRate.compareTo(new BigDecimal(1.05))>=0){
                    middleScore = new BigDecimal(110);
                }else if ( achieveRate.compareTo(new BigDecimal(1.05))<0 && achieveRate.compareTo(new BigDecimal(1))>=0) {
                    middleScore = new BigDecimal(100);
                } else if ( achieveRate.compareTo(new BigDecimal(1))<0 && achieveRate.compareTo(new BigDecimal(0.9))>=0 ) {
                    middleScore = new BigDecimal(90);
                }else if( achieveRate.compareTo(new BigDecimal(0.9))<0 && achieveRate.compareTo(new BigDecimal(0.8))>=0){
